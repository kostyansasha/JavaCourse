        ...
        try {
            ...
            for (Task t : tasks){
                ...
                if (!t.isRepeated()) { // if task repeat
                    ...
                    //hours
                    if (interOld >= 3600){
                        if (flag)  bufOut.append(" "); //If a previous condition is do, then put a space between them
                        else flag = true;

                        interNew = interOld / 3600;
                        if (interNew == 1) bufOut.append("1 hour");
                        else bufOut.append(interNew + " hours");
                        interOld = interOld - (interNew * 3600);
                    }
                    //minute
                    if (interOld >= 60){
                        if (flag)  bufOut.append(" ");
                        else flag = true;

                        interNew = interOld / 60;
                        if (interNew == 1) bufOut.append("1 minute");
                        else bufOut.append(interNew + " minutes");
                        interOld = interOld - (interNew * 60);
                    }
                    //second
                    if (interOld > 0){
                        if (flag)  bufOut.append(" ");
                        if (interOld == 1) bufOut.append("1 second");
                        else bufOut.append(interOld + " seconds");
                    }
                    bufOut.append("]");
                }
                // if inactive
